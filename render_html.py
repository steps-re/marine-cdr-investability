...
FIG_AT = {  # anchor text (in manuscript) -> figure file, caption
    ...
    "fig_waterbody_efficiency": ("fig_waterbody_efficiency.png", "Figure 10. First-order equilibration efficiency for water bodies outside the open-ocean atlas domain (Appendix I.1). The binding constraint changes by class: enclosed seas equilibrate fully (limit is source/durability), rivers and engineered outfalls are flushing/re-evasion-limited. Freshwater buffering recomputed per body."),
    "fig_crediting_gap": ("fig_crediting_gap.png", "Figure 11. The crediting-versus-realization gap (Appendix I.2, log scale). ~578,000 t contracted -> ~5,000 t issued -> ~2,800 t (~0.5%) physically realized once the equilibration lag is applied. Registry-verified data (Isometric, mid-2026)."),
    "fig_climate_corr": ("fig_climate_corr.png", "Figure 12. Interannual efficiency anomaly versus climate modes (Appendix I.3). Variability is real (~10-20% CV) but only weakly predictable: best correlations are a modest NAO signal at North Atlantic coastal sites; presented as a largely irreducible delivery risk, not forecastable skill."),
    "fig_flushing_diagnostic": ("fig_flushing_diagnostic.png", "Figure 13. Site flushing time from Lagrangian survival (Appendix I.4), 0.9-159 yr. It is set by flow geometry, not current speed: Duck NC has the fastest current yet moderate flushing (recirculating), the North Sea the fastest flushing (through-flow)."),
    "fig_three_panel": ("fig_three_panel.png", "Figure 14. Global surface current speed, equilibration time tau_eq, and Lagrangian flushing time. The three controls are spatially decorrelated (speed-flushing r=-0.04, tau_eq-flushing r=0.06): no single ocean property identifies viable sites (Appendix I.4)."),
    "fig_drifter_validation": ("fig_drifter_validation.png", "Figure 15. Independent validation against the NOAA Global Drifter Program climatology (Laurindo et al. 2017): modeled surface speed vs observed drifter speed, r=0.78 (Appendix I.5)."),
